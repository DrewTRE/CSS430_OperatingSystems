 ...
 * This Project is Scheduler.java , my rigid RoundRobin implementation
 */
    ...
    public void run() 
	{
        ...
        while (true) 
		{
            try 
			{
				...
				synchronized(queue)
				{
                if (queue.size() == 0) // check Queue size 
                    continue;
                currentTCB = (TCB)queue.firstElement();
				}
                if (currentTCB.getTerminated() == true) 
				{
                synchronized(queue)	// Queue synchronized at this point
				{
				...
				{
                    if (current.isAlive())
                        current.resume();	// added for rigid RR part1
                    else {
                ...
                synchronized (queue) 
				{
                    if (current != null && current.isAlive())
                        current.suspend();	// added for rigid RR part 1
